...
} = (await import('@fuxxy-star/baileys')).default
...
/**
    ...
    */
    sendPoll: {
        ...
        // >>> MODIFIKASI: MENAMBAHKAN FUNGSI KUSTOM adReply <<<
        adReply: {
            async value(jid, text, title = '', body = '', buffer, source = '', quoted, options) {
                // Jika buffer tidak null, kita asumsikan itu adalah thumbnail.
                let thumbData = null;
                if (buffer) {
                    try {
                        ...
                        console.error("Failed to get file for adReply thumbnail:", e);
                    }
                }
                ...
                return conn.sendMessage(jid, { text: text,
                    contextInfo: {
                        ...
                        externalAdReply: {
                            ...
                            title: title,
                            body: body,
                            // Menggunakan URL default seperti yang ada di referensi kode lama Anda
                            thumbnailUrl: 'https://telegra.ph/file/dc229854bebc5fe9ccf01.jpg', 
                            renderLargerThumbnail: true,
                            sourceUrl: source,
                            // Jika ada data thumbnail (buffer), gunakan
                            ...(thumbData ? { thumbnail: thumbData } : {})
                        }
                    }
                }, { quoted: quoted, ...options, ephemeralExpiration: 8600 })
            },
            enumerable: true
        },
        // >>> AKHIR MODIFIKASI adReply <<<
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        text: {
            get() {
                const msg = this.msg;
                const message = this.message; // Ambil properti message untuk mengakses balasan tombol
                
                // 1. Ambil teks/caption standar
                let text = (typeof msg === 'string' ? msg : msg?.text) || 
                           msg?.caption || 
                           msg?.contentText || 
                           '';

                // --- LOGIKA EKSTRAKSI ID TOMBOL/LIST MESSAGE (FIX) ---
                
                // 2. ID dari Quick Reply Button/Flow Message (msg?.selectedButtonId)
                if (msg?.selectedButtonId) {
                    text = msg.selectedButtonId;
                }
                
                // 3. ID dari Balasan Button Message (Jenis lama/deprecated)
                if (message?.buttonsResponseMessage?.selectedButtonId) {
                    text = message.buttonsResponseMessage.selectedButtonId;
                }
                
                // 4. ID dari Balasan Template Button (Jenis lama)
                if (message?.templateButtonReplyMessage?.selectedId) {
                    text = message.templateButtonReplyMessage.selectedId;
                }
                
                // 5. ID dari Balasan List Message (Penting untuk menu List Message)
                if (message?.listResponseMessage?.singleSelectReply?.selectedRowId) {
                    text = message.listResponseMessage.singleSelectReply.selectedRowId;
                }

                // 6. ID dari Balasan Interactive Message (Penting untuk Product Message/Button modern)
                if (message?.interactiveResponseMessage?.selectedId) {
                    text = message.interactiveResponseMessage.selectedId;
                }

                // 7. Jika menggunakan NativeFlow/Interactive Message, coba parse JSON
                if (msg?.nativeFlowResponseMessage) {
                    try {
                        ...
                        // Abaikan error parsing JSON jika tidak valid
                    }
                }
        ...
        // >>> MODIFIKASI: FUNGSI m.reply KUSTOM DARI KODE LAMA <<<
        reply: {
            value: async function (pesan, options) {
                const conn = this.conn;
                const m = this;
                // m.sender tidak selalu memiliki PP/status, tapi kode lama Anda mencoba mengambilnya
                const ppUrl = await conn
                    .profilePictureUrl(m.sender, "image")
                    .catch((_) => "https://telegra.ph/file/1ecdb5a0aee62ef17d7fc.jpg");
                let bio;
                try {
                    // Cek apakah conn.fetchStatus tersedia (fungsi ini mungkin tidak ada di Baileys baru)
                ...
                // FAKEKONTAK KUSTOM DARI simple.js LAMA ANDA
                const fkontak = {
                    key: {
                            participant: `0@s.whatsapp.net`,
                            // Menggunakan m.chat karena kode lama Anda menggunakan 'm' sebagai message object
                            ...(m.chat ? { remoteJid: `status@broadcast` } : {})
                        },
                        message: {
                            'contactMessage': {
                                'displayName': `Furina Terverifikasi oleh WhatsApp`,
                            }
                        }
                    };
                const a = {
                ...
                try {
                    // Logic lama Anda: jika ada options DAN pesan (diasumsikan options adalah path/buffer untuk file)
                    if (options && pesan) {
                        conn.sendFile(m.chat, options, null, pesan, m, null, a);
                    } else {
                        if (pesan) {
                            // Jika hanya ada pesan, kirim pesan dengan quote ke m
                            conn.reply(m.chat, pesan, m, a);
                        } else {
                            // Jika pesan kosong, asumsikan options adalah teksnya
