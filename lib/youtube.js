...
const download = {
  V1: async function dl(url, opts = {}) {
    try {
      ...
      if (!format) return { status: false, msg: 'Invalid type specified.' }
      ...
      const cleanedCookies = cookies.map(c => c.split(';')[0]).join('; ')
      ...
      const selectedFormat = analysisResult.formats[type].find(
        v => parseInt(v.quality) === quality
      )

      if (!selectedFormat) return { status: false, msg: 'No matching quality found.' }
      ...
      if (!downloadUrl) return { status: false, msg: 'No download link found.' }
    ...
    try {
      const res = await fetch(
        `https://ab.cococococ.com/ajax/download.php?copyright=0&format=${
          type === 'audio' ? 'mp3' : quality
        }&url=${url}&api=dfcb6d76f2f6a9894gjkege8a4ab232222`,
        { headers }
      )
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        const progressRes = await fetch(
          `https://p.oceansaver.in/ajax/progress.php?id=${data.id}`,
          { headers }
        )
