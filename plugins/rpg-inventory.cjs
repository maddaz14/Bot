...
const fetch = require('node-fetch'); // Jika diperlukan untuk gambar
...
let handler = async (m, { conn, usedPrefix, command }) => {
    ...
    let pp = await conn.profilePictureUrl(who, 'image').catch(() => global.fotorpg || './src/avatar_contact.png');
...
> © ${global.namebot || 'Bot'} 2025 ✨`.trim();
    ...
    // --- Tombol interaktif (contoh, bisa disesuaikan) ---
    let interactiveButtons = [
        {
            name: 'quick_reply',
            buttonParamsJson: JSON.stringify({
                display_text: '💰 Cek Bank',
                id: `${usedPrefix}bank` // Ganti dengan command bank Anda
            })
        },
        {
            name: 'quick_reply',
            buttonParamsJson: JSON.stringify({
                display_text: '⚔️ Cek profile',
                id: `${usedPrefix}me` // Ganti dengan command stats Anda
            })
        }
    ];
    ...
    await conn.sendMessage(m.chat, {
        product: {
            productImage: { url: pp }, // Gambar utama produk (profil user)
            productId: '9999999999999999',
            title: `Inventori ${name}`, // Judul di dalam kotak produk
            description: productDescription, // Rangkuman di dalam kotak produk
            currencyCode: 'IDR',
            priceAmount1000: '0',
        ...
        businessOwnerJid: '6285147777105@s.whatsapp.net', // **GANTI DENGAN JID WA BUSINESS BOT ANDA!**
        caption: fullInventoryCaption, // Detail lengkap inventori di bagian paling atas
        title: `✨ Detail Inventori ${name} ✨`, // Judul di bawah caption, di atas kotak produk
        subtitle: `Level: ${user.level} | Uang: Rp ${user.money.toLocaleString('id-ID')}`, // Subtitle di bawah judul di atas kotak produk
        footer: `> © ${global.namebot || 'Bot'} | RPG System`, // Footer di bawah tombol
