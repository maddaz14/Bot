const fkontak = {
    ...
    "message": {
        "conversation": `fightkucing ${global.namebot || 'Bot'} ✨`,
    }
};
...
conn.level = global.db.data.users[m.sender]
  ...
  if (typeof conn.fightnaga[m.sender] != "undefined" && conn.fightnaga[m.sender] == true) return conn.reply(m.chat, `*Tidak Bisa Melakukan Battle Karena Arena Yang Kamu Miliki Sedang Kamu Pakai Untuk Pet Yang Lain .*`, fkontak) // Ditambahkan fkontak
  ...
  conn.reply(m.chat, `*Pet Kamu* (Kucing ${global.db.data.users[m.sender].kucing}) Menantang Kucingnya *${conn.getName(lawan)}* (Kucing ${global.db.data.users[lawan].kucing}) Lagi Kelahi Rebutin Bini.\n\nTunggu ${lamaPertarungan} Menit Lagi Dan Lihat Siapa Yang Menang.`, fkontak) // Ditambahkan fkontak
