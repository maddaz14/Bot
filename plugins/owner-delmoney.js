...
let handler = async (m, { conn, args, isOwner, isMods, participants }) => {
  ...
  // Ambil jumlah money yang mau dikurangi
  let jumlah = 1000;
  ...
  // Kurangi money, tapi jangan sampai minus
  users[targetJid].money = Math.max(0, users[targetJid].money - jumlah);
  ...
  await conn.sendMessage(m.chat, {
    text: `✅ Sukses mengurangi *${jumlah.toLocaleString()}* money dari:\n👤 Nama: ${name}\n📱 Nomor/JID: ${targetJid}\n💰 Sisa: *${users[targetJid].money.toLocaleString()}*`,
    mentions: [targetJid]
  }, { quoted: m });
};

handler.help = ['delmoney <tag/nomor/JID> [jumlah]'];
handler.tags = ['owner'];
