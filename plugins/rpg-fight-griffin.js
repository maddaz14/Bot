const fkontak = {
    ...
    "message": {
        "conversation": `fightgriffin ${global.namebot || 'Bot'} ✨`,
    }
};
...
conn.level = global.db.data.users[m.sender]
  ...
  if (typeof conn.fightnaga[m.sender] != "undefined" && conn.fightnaga[m.sender] == true) return conn.reply(m.chat, `*Tidak Bisa Melakukan Battle Karena Arena Yang Kamu Miliki Dipakai Untuk Pertarungan Pet Kamu Yang Lain.*`, fkontak) // Ditambahkan fkontak
  ...
  conn.reply(m.chat, `*Pet Kamu* (Griffin ${global.db.data.users[m.sender].griffin}) Menantang Griffinnya *${conn.getName(lawan)}* (Griffin ${global.db.data.users[lawan].griffin}) Lagi Berkelahi.\n\nTunggu ${lamaPertarungan} Menit Lagi Dan Lihat Siapa Yang Menang.`, fkontak) // Ditambahkan fkontak
  ...
  if (pointPemain > pointLawan){
    ...
    conn.reply(m.chat, `*${conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${conn.getName(lawan)}*\n\nHasil imbang Kak, Ga Dapet Apa Apa`, fkontak) // Ditambahkan fkontak
  }
