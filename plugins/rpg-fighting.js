const fkontak = {
    ...
    "message": {
        "conversation": `fighting ${global.namebot || 'Bot'} ✨`,
    }
};
...
let handler = async (m, { conn, usedPrefix, participants }) => {
    ...
    if (typeof conn.misi[id] != "undefined" && conn.misi[id] == true) return conn.reply(m.chat, `Kamu masih bertarung.`, fkontak) // Ditambahkan fkontak
    ...
    conn.reply(m.chat, `*Kamu* (level ${user.level}) menantang *@${conn.getName(lawan)}* (level ${global.db.data.users[lawan].level}) dan sedang dalam pertarungan.\n\nTunggu 5 menit lagi dan lihat siapa yg menang.`, fkontak) // Ditambahkan fkontak
    conn.misi[id] = true
