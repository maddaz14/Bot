const fkontak = {
    ...
    "message": {
        "conversation": `warteg ${global.namebot || 'Bot'} ✨`,
    }
};
...
let handler = async (m, { conn }) => {
    ...
    if (!wartegs[chatId][user]) {
        wartegs[chatId][user] = {
            isOpen: false,
            currentOrder: null,
            money: 0, // PERUBAHAN: eris diubah menjadi money
            exp: 0,
            inventory: {
                common: [],
                uncommon: []
...
Ketik .pesanan <makanan>`, fkontak); // Menggunakan fkontak

        wartegs[chatId][user].timer = setTimeout(() => {
            if (wartegs[chatId][user].isOpen && wartegs[chatId][user].currentOrder) {
                conn.reply(chatId, `Info @${user.replace(/@.+/, '')}, Warteg kamu tutup karena tidak melayani pelanggan tepat waktu.`, fkontak); // Menggunakan fkontak
    ...
    if (command === '.warteg') {
        if (wartegs[chatId][user].isOpen) {
            conn.reply(chatId, `Hey @${user.replace(/@.+/, '')}, Warteg kamu masih buka. Silakan layani pelanggan.`, fkontak); // Menggunakan fkontak
            return;
        }
        ...
        if (currentTime - global.db.data.users[user].lastClosed < cooldown) {
            ...
            conn.reply(chatId, `@${user.replace(/@.+/, '')}, Kamu sudah melayani pelanggan seharian, istirahatlah sejenak\n${Math.ceil(remainingTime / 60000)} menit lagi.`, fkontak); // Menggunakan fkontak

            // Set a timeout to notify the user when the cooldown period is over
            setTimeout(() => {
                conn.reply(chatId, `@${user.replace(/@.+/, '')}, Kamu sudah bisa buka warteg lagi! Ketik .warteg untuk memulai.`, fkontak); // Menggunakan fkontak
            }, remainingTime);
            return;
        }
        ...
        if (!wartegs[chatId][user].isOpen) {
            conn.reply(chatId, `Aduh @${user.replace(/@.+/, '')}, Warteg kamu belum buka. Ketik .warteg untuk mulai`, fkontak); // Menggunakan fkontak
            return;
        }

        if (!wartegs[chatId][user].currentOrder) {
            conn.reply(chatId, `Sabar @${user.replace(/@.+/, '')}, Tidak ada pelanggan saat ini. Tunggu pelanggan baru datang.`, fkontak); // Menggunakan fkontak
            return;
        }

        let userOrder = m.text.split(' ').slice(1).join(' ').trim().toLowerCase();
        if (!userOrder) {
            conn.reply(chatId, 'Kamu harus menyertakan nama makanan setelah .pesanan.', fkontak); // Menggunakan fkontak
            return;
        }

        let rewardMoney = 0; // PERUBAHAN: rewardEris diubah menjadi rewardMoney
        let rewardExp = 0;
        ...
        wartegs[chatId][user].currentOrder = null;
        wartegs[chatId][user].money += rewardMoney; // PERUBAHAN: eris diubah menjadi money
        ...
        global.db.data.users[user].money += rewardMoney; // PERUBAHAN: eris diubah menjadi money
        ...
        if (rewardMoney >= 0) {
            ...
            global.db.data.users[user].common = (global.db.data.users[user].common || 0) + rewardCommon; // Tambahkan ke global.db.data.users
            global.db.data.users[user].uncommon = (global.db.data.users[user].uncommon || 0) + rewardUncommon; // Tambahkan ke global.db.data.users
        }
        ...
        let formattedMoney = rewardMoney.toLocaleString(); // PERUBAHAN: formattedEris diubah menjadi formattedMoney
...
Tunggu 3 menit untuk pelanggan berikutnya`;
        ...
        conn.reply(chatId, message, fkontak); // Menggunakan fkontak

        // Automatically close the warteg if 5 customers are served
        if (wartegs[chatId][user].customerCount >= 5) {
            conn.reply(chatId, `@${user.replace(/@.+/, '')}, Warteg kamu tutup karena sudah malam setelah melayani 5 pelanggan. Kamu bisa membuka lagi dalam 30 menit.`, fkontak); // Menggunakan fkontak
            clearInterval(wartegs[chatId][user].interval);
            clearTimeout(wartegs[chatId][user].timer);
            wartegs[chatId][user].isOpen = false;
            wartegs[chatId][user].currentOrder = null;
            ...
            global.db.data.users[user].money += wartegs[chatId][user].money; // PERUBAHAN: eris diubah menjadi money
            global.db.data.users[user].exp += wartegs[chatId][user].exp;

            if (wartegs[chatId][user].money >= 0) { // PERUBAHAN: eris diubah menjadi money
                // Item common dan uncommon sudah ditambahkan di bagian atas, ini mungkin duplikasi jika rewardCommon/Uncommon juga ditambahkan ke inventori global
                // saya akan berasumsi ini untuk total yang ada di warteg[chatId][user].inventory.common/uncommon
                if (!global.db.data.users[user].inventory) {
                    global.db.data.users[user].inventory = { common: [], uncommon: [] };
                }
                if (!global.db.data.users[user].inventory.common) {
                ...
                // Jika ingin menambahkan jumlah total, bukan array:
                // global.db.data.users[user].common = (global.db.data.users[user].common || 0) + wartegs[chatId][user].inventory.common.reduce((sum, val) => sum + val, 0);
                // global.db.data.users[user].uncommon = (global.db.data.users[user].uncommon || 0) + wartegs[chatId][user].inventory.uncommon.reduce((sum, val) => sum + val, 0);
            }

            // Reset user warteg data
            wartegs[chatId][user].money = 0; // PERUBAHAN: eris diubah menjadi money
        ...
        if (!wartegs[chatId][user].isOpen) {
            conn.reply(chatId, 'Warteg kamu sudah tutup atau belum buka.', fkontak); // Menggunakan fkontak
            return;
        }
        clearInterval(wartegs[chatId][user].interval);
        clearTimeout(wartegs[chatId][user].timer);
        ...
        wartegs[chatId][user].customerCount = 0;
        global.db.data.users[user].lastClosed = Date.now();
        conn.reply(chatId, 'Warteg ditutup.', fkontak); // Menggunakan fkontak
    }
};
