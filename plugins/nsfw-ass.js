...
const API_URL = 'https://api.nekolabs.my.id/random/nsfwhub/ass';
...
let handler = async (m, { conn }) => {
    ...
    try {
        ...
        await conn.sendMessage(m.chat, {
            image: imageBuffer,
            caption: '🔞 Gambar NSFW (Ass) siap! (via Nekolabs)'
        }, {
            quoted: m
        });
        ...
        console.error('Error in nsfw-ass handler:', e);
        await conn.sendMessage(m.chat, { react: { text: '❌', key: m.key } });
        await conn.reply(m.chat, `💥 Terjadi kesalahan: ${e.message}`, m);
    }
}

handler.help = ['assnsfw'];
handler.tags = ['nsfw']; 
handler.command = /^(assnsfw|ass)$/i;
