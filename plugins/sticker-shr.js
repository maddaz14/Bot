...
const handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if (text.length > 300) return m.reply(`Karakter terbatas kalimat`);
  ...
  try {
    for (let i = 0; i < words.length; i++) {
      ...
      const res = await axios.get(
        `https://api.botcahx.eu.org/api/ephoto/horor?text=${encodeURIComponent(currentText)}&apikey=jagaamanah`,
...
handler.help = ["shoror"].map((v) => v + " *text*");
handler.tags = ["sticker"];
handler.command = ["shoror", "shr"];
